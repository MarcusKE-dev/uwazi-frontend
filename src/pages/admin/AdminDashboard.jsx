            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-blue-900/50">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {unassigned.map((c) => (
                    <tr key={c.id} className="border-t border-slate-100 dark:border-blue-900/20 hover:bg-slate-50 dark:hover:bg-uwazi-mid/30">
                      <td className={`${cellCls} font-mono text-xs text-emerald-600 dark:text-emerald-400 font-semibold`}>
                        {c.tracking_code}
                      </td>
                      <td className={cellCls}>
                        <p className="max-w-xs truncate">{c.title}</p>
                      </td>
                      <td className={`${cellCls} text-xs`}>{c.ministry ?? '—'}</td>
                      <td className={`${cellCls} text-xs`}>{fmtDateTime(c.created_at)}</td>
                      <td className={cellCls}>
                        <select
                          value={selectedReviewer[c.id] ?? ''}
                          onChange={(e) =>
                            setSelectedReviewer((prev) => ({ ...prev, [c.id]: e.target.value }))
                          }
                          className="text-xs border border-slate-200 dark:border-blue-900/50 rounded-lg px-2 py-1.5 bg-white dark:bg-uwazi-dark outline-none min-w-[160px]"
                        >
                          <option value="">Select reviewer…</option>
                          {reviewers.map((r) => (
                            <option key={r.id} value={r.id}>
                              {r.full_name} ({r.active_cases} active)
                            </option>
                          ))}
                        </select>
                      </td>
                      <td className={cellCls}>
                        <button
                          onClick={() => {
                            const rid = selectedReviewer[c.id];
                            if (!rid) { toast.error('Select a reviewer first'); return; }
                            assignCase.mutate({ caseId: c.id, reviewerId: rid });
                          }}
                          disabled={assignCase.isPending || !selectedReviewer[c.id]}
                          className="px-3 py-1.5 bg-uwazi-navy hover:bg-uwazi-mid disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-lg transition-colors"
                        >
                          Assign
                        </button>
                      </td>
